...
describe('3dvr-web logo branding', () => {
  ...
  it('ships a touch and mouse driven Three.js token for the homepage logo', async () => {
    ...
    assert.match(token, /CylinderGeometry\(1\.45, 1\.45, 0\.18/);
    ...
    assert.match(token, /fillText\('3dvr', 0, 10\)/);
    assert.match(token, /fillText\('3dvr', -5, -5\)/);
    assert.match(token, /minted bevel/);

...
describe('3dvr-web customer journey copy', () => {
  it('keeps the homepage focused on concrete first steps and the portal start path', async () => {
    ...
    assert.doesNotMatch(html, /Simple ways to work together/);
    ...
    assert.doesNotMatch(html, /Family &amp; Friends/);
    assert.doesNotMatch(html, /\$5 monthly/);
    assert.doesNotMatch(html, /Light monthly support for people close to the work who want to help keep 3dvr moving\./);
    assert.doesNotMatch(html, /For launching your product, brand, or first offer immediately\./);
    assert.doesNotMatch(html, /Choose Plan/);
    ...
    assert.doesNotMatch(html, /Default for active businesses that need a site, follow-up, updates, and calmer operations\./);
    assert.doesNotMatch(html, /Pair it with a setup sprint when you want the first workflow built faster\./);
    ...
    assert.doesNotMatch(html, /Portal command center/);
    assert.doesNotMatch(html, /Start in the browser with one identity, one launcher, and one calmer operating lane\./);
    assert.doesNotMatch(html, /Portal layer/);
    assert.doesNotMatch(html, /OS direction/);
    assert.doesNotMatch(html, /See full system map/);
  ...
  it('moves the full Portal Browser OS system map onto its own page', async () => {
    ...
    assert.match(html, /<section id="command-center"/);
